...
x0 = [100,0,100,0]';          % initial state for data generation
...
%         R_k = ctr_cov_sensor_pos(apbm_xPred, R , sen_var*eye(2));
%         if sum(isnan(R_k))>0
%             R_k = R;
%             disp('APBM diverged')
%         end
...
%         R_k = ctr_cov_sensor_pos(nn_xPred, R , sen_var*eye(2));
%         if sum(isnan(R_k))>0
%             R_k = R;
%             disp('NN diverged')
%         end
...
legend('True', 'APBM', 'NN','PBM','TM','Sensor','Location','southeast', 'fontsize', fontsize-2)
...
legend('TRM', 'APBM', 'NN', 'PBM', 'fontsize', fontsize-2, 'location','best')
ax = gca; ax.FontSize = fontsize-2;
...
%%
h5 = figure;
a = cdfplot(sum((save_tm_ckf_x_mmse - save_x).^2,2));
set(a, 'LineStyle', ':', 'LineWidth', 2)
hold on
a = cdfplot(sum((save_apbm_ckf_x_mmse - save_x).^2,2));
set(a, 'LineStyle', '-', 'LineWidth', 2)
% set(a, 'Marker', 'square')
a = cdfplot(sum((save_nn_ckf_x_mmse - save_x).^2,2));
set(a, 'LineStyle', '--', 'LineWidth', 2)
a = cdfplot(sum((save_ckf_x_mmse - save_x).^2,2));
set(a, 'LineStyle', '-.', 'LineWidth', 2)
legend('TRM','APBM','NN','PBM','fontsize', fontsize-2, 'location','best')
